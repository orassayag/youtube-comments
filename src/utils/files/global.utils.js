...
class GlobalUtils {
    ...
    isPathExistsError(targetPath) {
        ...
        if (!fs.existsSync(targetPath)) {
            throw new Error(`targetPath not exists: ${targetPath} (1000032)`);
        }
    }
    ...
    isPathAccessible(targetPath) {
        ...
        if (fs.accessSync(targetPath, fs.constants.R_OK)) {
            throw new Error(`targetPath not readable: ${targetPath} (1000033)`);
        }
        // Check if the path is writable.
        if (fs.accessSync(targetPath, fs.constants.W_OK)) {
            throw new Error(`targetPath not writable: ${targetPath} (1000034)`);
        }
    }
}
