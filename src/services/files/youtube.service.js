...
class YoutubeService {
  ...
  async getAPIKey(data) {
    ...
    if (!fileUtils.isFilePath(path)) {
      throw new Error(
        `The parameter path ${parameterName} marked as file but it's a path of a directory: ${path} (1000014)`
      );
    }
    const extension = pathUtils.getExtension(path);
    if (extension !== fileExtension) {
      throw new Error(
        `The parameter path ${parameterName} must be a ${fileExtension} file but it's: ${extension} file (1000015)`
      );
    }
  ...
  async validateVideoId() {
    ...
    if (!response || !response.data) {
      throw new Error(
        `Video ${this.youtubeDataModel.videoId} not exists via YouTube API details call (1000019)`
      );
    }
    ...
    if (!validationUtils.isPropertyExists(response.data.items[0], 'id')) {
      throw new Error(
        'Missing `items.id` field in the JSON response (1000022)'
      );
    }
    const responseVideoId = response.data.items[0].id.trim();
    if (this.youtubeDataModel.videoId !== responseVideoId) {
      throw new Error(
        `Mismatch video Ids between settings and API response: ${this.youtubeDataModel.videoId} | ${responseVideoId} (1000023)`
      );
    }
  ...
  getCommentsCount(videoDetails) {
    if (!validationUtils.isPropertyExists(videoDetails, 'statistics')) {
      throw new Error(
        'Missing `items.statistics` field in the JSON response (1000024)'
      );
    }
    if (
      !validationUtils.isPropertyExists(videoDetails.statistics, 'commentCount')
    ) {
      throw new Error(
        'Missing `items.statistics.commentCount` field in the JSON response (1000025)'
      );
    }
    this.youtubeDataModel.commentsCount = parseInt(
  ...
  async fetchComments() {
    ...
    while (isFetchComments) {
      const url = `${this.youtubeDataModel.apiBaseURL}${this.youtubeDataModel.videoCommentsQuery
        .replace(PlaceholderEnum.VIDEO_ID, this.youtubeDataModel.videoId)
        .replace(
          PlaceholderEnum.MAX_RESULTS,
          countLimitService.countLimitDataModel.maximumFetchCommentsCount
        )
      ...
      if (!response || !response.data) {
        throw new Error(
          `Video ${this.youtubeDataModel.videoId} not exists via YouTube API details call (1000026)`
        );
      }
      if (!validationUtils.isPropertyExists(response.data, 'items')) {
      ...
      for (let i = 0; i < comments.length; i++) {
        ...
        if (
          this.youtubeDataModel.commentsIndex >=
          this.youtubeDataModel.commentsCount
        ) {
          isFetchComments = false;
          break;
        }
        if (
          this.youtubeDataModel.commentsIndex >=
          countLimitService.countLimitDataModel.maximumCommentsCount
        ) {
          isFetchComments = false;
      ...
      await globalUtils.sleep(
        countLimitService.countLimitDataModel.millisecondsFetchDelayCount
      );
    }
  ...
  async logComment(commentContainer) {
    ...
    if (
      !validationUtils.isPropertyExists(
        commentContainer.snippet.topLevelComment,
        'snippet'
      )
    ) {
      return;
    ...
    if (
      !validationUtils.isPropertyExists(
        commentContainer.snippet.topLevelComment.snippet,
        'textOriginal'
      )
    ) {
      return;
    }
    await logService.logComment(
      commentContainer.snippet.topLevelComment.snippet.textOriginal
    );
  }
