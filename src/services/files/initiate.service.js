...
class InitiateService {
  ...
  validatePositiveNumbers() {
    [
      // ===COUNT & LIMIT=== //
      'MAXIMUM_COMMENTS_COUNT',
      'MILLISECONDS_FETCH_DELAY_COUNT',
      'MAXIMUM_FETCH_COMMENTS_COUNT',
  ...
  validateSpecial() {
    ...
    if (!validationUtils.isValidYouTubeId(VIDEO_ID)) {
      throw new Error(
        `Invalid or no VIDEO_ID parameter was found: Expected a YouTube video Id but received: ${VIDEO_ID} (1000008)`
      );
    }
    // ===COUNT & LIMIT=== //
    if (
      MAXIMUM_FETCH_COMMENTS_COUNT <= 0 ||
      MAXIMUM_FETCH_COMMENTS_COUNT > 100
    ) {
      throw new Error(
        `Invalid or no MAXIMUM_FETCH_COMMENTS_COUNT parameter was found: Needs to be between 1 to 100 ${VIDEO_ID} (1000009)`
      );
    }
    [
      // ===GENERAL=== //
      ...
      if (!validationUtils.isValidLink(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a URL but received: ${value} (1000010)`
        );
      }
    });
  }
  ...
  validateDirectories() {
    ...
    [
      ...
      if (!fileUtils.isDirectoryPath(value)) {
        throw new Error(
          `The parameter path ${key} marked as directory but it's a path of a file: ${value} (1000011)`
        );
      }
    });
  }
